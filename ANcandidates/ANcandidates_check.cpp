...
struct data {
  ...
  bool operator==(const data &other) {
    return (n == other.n) && (k == other.k) && (a == other.a) && (sdr == other.sdr) &&
           (numCandidates == other.numCandidates);
  }

  bool operator<(const data &other) { return (k == other.k && a < other.a) || k < other.k; }
  ...
  bool compare_to(const data &other, std::ostream &out) const {
    ...
    if (numCandidates != other.numCandidates) {
      out << (isOK ? "" : " ") << "numCandidates (" << numCandidates << ") != other.numCandidates("
          << other.numCandidates << ')';
      isOK = false;
    }
    if (isOK) {
  ...
  bool compare_to(const data &other) const { return compare_to(other, std::clog); }
...
struct hash<::data> {
  ...
  // static constexpr const result_type multiplier =
  // std::numeric_limits<ssize_t>::max() >> (sizeof(ssize_t) * 4); // OK, let's
  // assume we never get to such large data lengths and widths of A

  // OK, let's assume we never get to such large data lengths and widths of A
  static constexpr const result_type multiplier = 128;
...
bool operator==(const ::data &d1, const ::data &d2) { return std::hash<::data>{}(d1) == std::hash<::data>{}(d2); }
bool operator<(const ::data &d1, const ::data &d2) { return std::hash<::data>{}(d1) < std::hash<::data>{}(d2); }

using MyMap = std::map<::data, std::vector<size_t>>;
...
void parse_candidates(std::istringstream &issLine, std::string &strCandidate, data &d, std::vector<size_t> &vec,
    ...
    for (size_t j = i + 1; j < file_streams.size(); ++j) {
      ...
      for (auto &entry1 : map1) {
        ...
        if (iter != map2.end()) {
          ...
          // we do not use all members of type data for the hash!
          bool is_same_data = entry1.first.compare_to(iter->first);
