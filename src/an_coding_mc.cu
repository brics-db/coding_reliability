...
extern double run_ancoding_mc(uintll_t n, uintll_t iterations, uintll_t A, int verbose, double* times, uintll_t* minb, uintll_t* mincb, int file_output)
{
  int nr_dev;
  Statistics stats;
...
#pragma omp parallel for num_threads(nr_dev) schedule(static,1)
  ...
  double max_abs_error = -1.0f;

  if(A==641)
    max_abs_error = get_rel_error_AN641(n, counts, 2);
  ...
  if(verbose)
  {
    ...
    printf("Max abs. error to solution: %lf\n", max_abs_error);
  }
